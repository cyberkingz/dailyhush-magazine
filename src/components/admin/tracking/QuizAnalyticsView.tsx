...
import { TrendingUp, TrendingDown, Download, Search, ClipboardList } from 'lucide-react'
...
import { QuizFunnelChart } from '../analytics/QuizFunnelChart'
import { FunnelActionItems } from '../analytics/FunnelActionItems'
...
export function QuizAnalyticsView({ dateRange: externalDateRange }: QuizAnalyticsViewProps) {
  ...
  const [internalDateRange, setInternalDateRange] = useState<ReactDayPickerDateRange | undefined>({
  ...
  const activeDateRange = useMemo(() => {
    if (externalDateRange) {
      return {
        from: new Date(externalDateRange.startDate),
        to: new Date(externalDateRange.endDate)
      }
    }
  ...
  useEffect(() => {
    ...
    activeDateRange?.from?.getTime(),
    activeDateRange?.to?.getTime(),
    comparisonMode
  ])
  ...
  async function loadAnalyticsData() {
    try {
      ...
      const analyticsDateRange = activeDateRange?.from && activeDateRange?.to
        ? {
            startDate: activeDateRange.from.toISOString(),
            endDate: activeDateRange.to.toISOString(),
          }
        : undefined
      ...
      const currentPeriodSubmissions = allSubmissions?.filter(s => {
        ...
        return activeDateRange?.from && activeDateRange?.to
          ? submissionDate >= activeDateRange.from && submissionDate <= activeDateRange.to
          : true
      }) || []
      ...
      if (comparisonMode && activeDateRange?.from && activeDateRange?.to) {
        const periodLength = differenceInDays(activeDateRange.to, activeDateRange.from)
        const previousPeriodEnd = subDays(activeDateRange.from, 1)
      ...
      const allCampaignMetrics = await getCampaignMetrics(analyticsDateRange)

      // Filter out Facebook/Instagram ad campaigns based on utm_source
      // Only show actual email campaigns in this view
      const emailOnlyCampaigns = allCampaignMetrics.filter(c => {
      ...
      // Set empty metrics to prevent stuck loading state
      setMetrics({
        totalViews: 0,
        totalStarts: 0,
        totalCompletions: 0,
  ...
  const hasNoData = metrics.totalViews === 0

  return (
    <div className="space-y-6">
      {/* Controls - Only show if not using external date range */}
      {!externalDateRange && (
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 w-full sm:w-auto">
          ...
          <BasicDateRangePicker
            date={internalDateRange}
            onDateChange={setInternalDateRange}
      ...
      {hasNoData ? (
        <GlassCard intensity="heavy">
          <CardContent className="py-20 text-center">
            <div className="max-w-md mx-auto">
              <div className="inline-flex items-center justify-center w-20 h-20 mb-6 rounded-full bg-gradient-to-br from-emerald-500/20 to-emerald-600/10 border border-emerald-500/20">
                <ClipboardList className="h-10 w-10 text-emerald-400" />
              </div>
              <h3 className="text-2xl font-bold text-white mb-3">No Quiz Data Yet</h3>
              <p className="text-base text-white/70 mb-6 leading-relaxed">
                No quiz activity has been recorded for the selected time period.
                {activeDateRange?.from && activeDateRange?.to && (
                  <span className="block mt-2 text-sm text-white/60">
                    Selected period: {format(activeDateRange.from, 'MMM d, yyyy')} - {format(activeDateRange.to, 'MMM d, yyyy')}
                  </span>
                )}
              </p>
              <div className="bg-white/5 rounded-lg p-4 border border-white/10 backdrop-blur-sm">
                <p className="text-sm text-white/80 mb-2 font-medium">What to expect here:</p>
                <ul className="text-sm text-white/60 space-y-1.5 text-left max-w-sm mx-auto">
                  <li className="flex items-start gap-2">
                    <span className="text-emerald-400 mt-0.5">•</span>
                    <span>Quiz performance metrics and conversion rates</span>
                  </li>
                  <li className="flex items-start gap-2">
                    <span className="text-emerald-400 mt-0.5">•</span>
                    <span>Visitor engagement and completion funnels</span>
                  </li>
                  <li className="flex items-start gap-2">
                    <span className="text-emerald-400 mt-0.5">•</span>
                    <span>Question-level analytics and drop-off rates</span>
                  </li>
                  <li className="flex items-start gap-2">
                    <span className="text-emerald-400 mt-0.5">•</span>
                    <span>Email capture rates and campaign performance</span>
                  </li>
                </ul>
              </div>
              <p className="text-xs text-white/50 mt-6">
                Data will appear automatically once visitors start taking the quiz
              </p>
            </div>
          </CardContent>
        </GlassCard>
        ...
        <>
          {/* All existing content when data is available */}
          ...
          <CardContent className="p-0">
            ...
            {comparisonMode && previousMetrics ? (
              ...
              <div className="flex items-center justify-between text-xs mt-1">
                ...
                <span className="font-medium text-emerald-400">{metrics.totalCompletions} completed</span>
              </div>
            )}
          </CardContent>
        </GlassCard>
