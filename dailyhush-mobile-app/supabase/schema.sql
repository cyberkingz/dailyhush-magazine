-- DailyHush Mobile App - Supabase Database Schema
...
-- ================================================
-- SHIFT DEVICE TABLE
-- The Shift necklace pairing and usage
-- ================================================
...
-- Track necklace usage for analytics
-- ================================================
...
-- Track module completion and exercises
-- ================================================
...
-- Spiral logs indexes
CREATE INDEX idx_spiral_logs_user_id ON public.spiral_logs(user_id);
CREATE INDEX idx_spiral_logs_timestamp ON public.spiral_logs(timestamp DESC);
CREATE INDEX idx_spiral_logs_user_timestamp ON public.spiral_logs(user_id, timestamp DESC);

-- Pattern insights indexes
CREATE INDEX idx_pattern_insights_user_id ON public.pattern_insights(user_id);
CREATE INDEX idx_pattern_insights_week ON public.pattern_insights(week_start DESC);

-- Shift usage indexes
CREATE INDEX idx_shift_usage_user_id ON public.shift_usage_logs(user_id);
CREATE INDEX idx_shift_usage_timestamp ON public.shift_usage_logs(timestamp DESC);

-- Voice journals indexes
CREATE INDEX idx_voice_journals_user_id ON public.voice_journals(user_id);
CREATE INDEX idx_voice_journals_timestamp ON public.voice_journals(timestamp DESC);
CREATE INDEX idx_voice_journals_not_deleted ON public.voice_journals(user_id) WHERE deleted_at IS NULL;
...
-- (This will be managed by Stripe in production)

-- ================================================
-- USAGE NOTES
-- ================================================

-- To create a new user profile after signup:
-- INSERT INTO public.user_profiles (user_id, email)
-- VALUES (auth.uid(), 'user@example.com');

-- To log a spiral:
-- INSERT INTO public.spiral_logs (user_id, duration_seconds, pre_feeling, post_feeling, technique_used)
-- VALUES (auth.uid(), 90, 3, 7, 'breathing');

-- To update FIRE training progress:
-- UPDATE public.user_profiles
-- SET fire_progress = jsonb_set(fire_progress, '{focus}', 'true')
-- WHERE user_id = auth.uid();

-- ================================================
-- CLEAN UP (Run this periodically)
-- ================================================

-- Soft delete voice journals older than 90 days
-- SELECT soft_delete_old_voice_journals();

-- Permanently delete soft-deleted journals after 30 days
-- DELETE FROM public.voice_journals
