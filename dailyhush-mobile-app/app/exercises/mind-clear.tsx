...
function MindClearStage({
  ...
  const [selectedCategory, setSelectedCategory] = useState<
    'worry' | 'task' | 'memory' | 'decision' | 'other' | null
  >(null);
  ...
  const handleAddThought = async () => {
    ...
    const updatedCategories = mindClearData.thoughtCategories.map((cat) =>
      cat.category === selectedCategory ? { ...cat, count: cat.count + 1 } : cat
    );
      ...
      <View className="space-y-6">
        ...
        <View className="bg-forest-800/50 border-forest-700/30 rounded-xl border p-4">
          ...
          <View className="bg-forest-900 h-2 overflow-hidden rounded-full">
            <View className="bg-mindful-teal h-full" style={{ width: `${progress}%` }} />
          </View>
        </View>
        ...
        <View className="space-y-3">
          <Text className="font-inter-semibold text-sm text-sage-300">
            1. What type of thought?
          </Text>
          <View className="flex-row flex-wrap gap-2">
            {(Object.keys(THOUGHT_CATEGORIES) as (keyof typeof THOUGHT_CATEGORIES)[]).map(
              (category) => {
                ...
                return (
                  <TouchableOpacity
                    ...
                    className={`min-w-[45%] flex-1 rounded-xl border-2 p-3 ${
                    ...
                    <View className="mb-1 flex-row items-center gap-2">
                      ...
                      <Text
                        className={`font-inter-semibold text-sm ${isSelected ? 'text-sage-50' : 'text-sage-300'}`}>
                        {config.label}
                      </Text>
                    </View>
                    <Text className="font-inter-regular text-xs text-sage-400">
                      {config.example}
                    </Text>
                  </TouchableOpacity>
                );
        ...
        {selectedCategory && (
          <View className="space-y-3">
            ...
            <TouchableOpacity
              ...
              <Text
                className={`font-poppins-semibold text-center text-base ${thoughtInput.trim() ? 'text-forest-900' : 'text-sage-600'}`}>
                Clear This Thought
              </Text>
            </TouchableOpacity>
          </View>
